...
JsonPrivate::JsonPrivate(const QString &jsonOrJsonFilePath, bool fromFile) {
    ...
    if (fromFile) {
        ...
        if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            json = file.readAll();
		#ifdef PRINT_JSON_FILE
			qDebug() << json.data();
		#endif
		#ifndef LOG_JSON_FILE_OFF
			INFO(json.data());
		#endif
    ...
    if (QJsonParseError::NoError == error.error) {
        ...
        valid = false;
        errorString = QString("%1:%2\nOffset: %3").arg(jsonOrJsonFilePath).arg(error.errorString()).arg(error.offset);
	#ifndef LOG_JSON_FILE_OFF
        WARN(errorString);
	#endif
    }
...
void JsonPrivate::removeRootKey(const QString &key)
{
    root.remove(key);
}
...
void Json::removeRootKey(const QString &key)
{
    d->removeRootKey(key);
}

// 把 JSON 保存到 path 指定的文件
void Json::save(const QString &path, std::function<QByteArray(const QByteArray &)> encryptFn, bool pretty) const {
    QFile file(path);
    if (!file.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        return;
    }
    QByteArray ba = QJsonDocument(d->root).toJson(pretty ? QJsonDocument::Indented : QJsonDocument::Compact);
    //qDebug() << ba;
    if(encryptFn){
        ba = encryptFn(ba);
    }
    qint64 size = file.write(ba);
    if(-1 == size){
        qWarning()<<"save json file failed, path:" << path;
    }
    file.close();
}
